...
async function main() {
  ...
  teacherSearch.setData({ activeLevel: 'L3' });
  teacherSearch.applyFilters.call(teacherSearch);
  ...
  await sleep();
  assert(
    reviewRecords.data.records.some((record) => record.statusText === '审核中'),
    `review records should map submitted status, got ${JSON.stringify(reviewRecords.data.records)}`,
  );
